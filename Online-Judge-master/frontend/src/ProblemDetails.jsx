...
function ProblemDetails() {
  ...
  async function Code_Run() {
    ...
    try {
      ...
      if (response.status === 200) {
        console.log("Setting output to:", response.data.output);
  ...
  async function Code_Run_Sample_TC() {
    // Use the sample test cases from the problem
    const sampleInput = problem.showtc;
    const data_to_send = { language: selectedLanguage, code: code, input: sampleInput };
    ...
    try {
      ...
      console.log("Sample TC Run Response:", response);

      if (response.status === 200) {
        setOutput(response.data.output || "No output generated");
        // Also show expected output for comparison
  ...
  Passed Test Cases: {countpassed}
            ...
            <div className="Item-2_prob">
              ...
              <div className="button-group">
                <button className="run-tc-button" onClick={Code_Run_Sample_TC}>
                  Run Sample TC
                </button>
