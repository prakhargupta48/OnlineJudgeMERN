...
app.post('/run', async (req, res) => {
  ...
  try {
    ...
    if (language === 'cpp') {
      ...
      // Clean up temporary files safely for Python
      try {
        if (fs.existsSync(filePath)) {
          await fs.promises.unlink(filePath);
          console.log(`Deleted file: ${filePath}`);
      ...
      // Clean up temporary files safely for Java
      try {
        if (fs.existsSync(filePath)) {
          await fs.promises.unlink(filePath);
          console.log(`Deleted file: ${filePath}`);
...
// Removed Firebase dependencies - now using MongoDB directly for storing test cases and outputs
...
app.post('/submit', async (req, res) => {
  ...
  try {
    // Get test cases and expected output directly from MongoDB
